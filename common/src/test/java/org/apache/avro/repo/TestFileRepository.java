...
public class TestFileRepository extends AbstractTestRepository<FileRepository> {
  ...
  protected FileRepository createRepository() {
    return new FileRepository(REPO_PATH);
  }
  ...
  public void testPathHandling() throws SchemaValidationException {
    ...
    for (String path : paths) {
      FileRepository r = new FileRepository(TEST_PATH + path);
      try {
    ...
    // verify idempotent
    new FileRepository(TEST_PATH + "/tmp/repo").close();
    new FileRepository(TEST_PATH + "/tmp/repo").close();
  }
  ...
  public void testReadWritten() throws SchemaValidationException {
    ...
    FileRepository r = new FileRepository(path);
    try {
  ...
  public void testInvalidDir() throws IOException {
    ...
    FileRepository r = new FileRepository(badPath);
    r.close();
  }

  @Test(expected = IllegalStateException.class)
  public void testCantUseClosedRepo() {
    FileRepository r = new FileRepository(TEST_PATH + "/tmp/repo");
    r.close();
    r.lookup("nothing");
  }
