...
public class RESTRepository {
   ...
   */
  @Inject
  public RESTRepository(@Named("avro.repo.cached") Repository repo) {
    this.repo = repo;
  }
   ...
   *          the identifier for the validator class
  ...
  public Response createSubject(@PathParam("subject") String subject,
      @FormParam("validator_class") String validatorClass) {
    if (null == subject) {
      return Response.status(400).build();
    }
    Subject created = repo.register(subject, validatorClass);
    if (null == created) {
      throw new ConflictException("Unable to create subject named " + subject
          + " with compatibility mode " + validatorClass);
    }
